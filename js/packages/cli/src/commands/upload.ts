import { EXTENSION_JSON, EXTENSION_PNG, EXTENSION_WAV } from '../helpers/constants';
...
import { arweaveUploadWithAudio, arweaveUpload } from '../helpers/upload/arweave';
...
export async function uploadWithAudio(
  files: string[],
  cacheName: string,
  env: string,
  keypair: string,
  ...
  files.forEach(f => {
    ...
    if (!seen[f.replace(EXTENSION_WAV, '').split('/').pop()]) {
      seen[f.replace(EXTENSION_WAV, '').split('/').pop()] = true;
      newFiles.push(f);
    }
  });
  existingInCache.forEach(f => {
  ...
  const images = newFiles.filter(val => path.extname(val) === EXTENSION_PNG);
  const audios = newFiles.filter(val => path.extname(val) === EXTENSION_WAV);
  const SIZE = images.length;
    ...
    chunks(Array.from(Array(SIZE).keys()), 1000).map(
      async allIndexesInSlice => {
        for (let ind = 0; ind < allIndexesInSlice.length; ind++) {
          ...
          let link = cacheContent?.items?.[index]?.link;
          let imageLink = cacheContent?.items?.[index]?.imageLink;
          let audioLink = cacheContent?.items?.[index]?.imageLink;
          if (!link || !cacheContent.program.uuid) {
            if (i >= lastPrinted + tick || i === 0) {
              lastPrinted = i;
              log.info(`Processing file: ${i}, ${imageName} - ${audioName}`);
            }
            const manifestPath = image.replace(EXTENSION_PNG, EXTENSION_JSON);
            const manifestContent = fs
              .readFileSync(manifestPath)
            ...
            if (!link) {
              try {
                if (storage === 'arweave') {
                  [link, imageLink, audioLink] = await arweaveUploadWithAudio(
                    walletKeyPair,
                    anchorProgram,
                    env,
                    image,
